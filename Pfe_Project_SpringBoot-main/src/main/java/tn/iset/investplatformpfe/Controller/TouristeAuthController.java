...
import tn.iset.investplatformpfe.Service.TouristeAuthService;
...
public class TouristeAuthController {

    private final TouristeAuthService authService;

    public TouristeAuthController(TouristeAuthService authService) {
    ...
    // ✅ Endpoint pour récupérer par ID (si nécessaire)
    @GetMapping("/{id}")
    public ResponseEntity<?> getTouristeById(@PathVariable Long id) {
        // Implémentez si nécessaire
        return ResponseEntity.ok(Map.of("message", "Touriste avec id: " + id));
    }
}